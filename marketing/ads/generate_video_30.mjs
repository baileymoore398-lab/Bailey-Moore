// RouteForge 30-second video ad (vertical 1080×1920) for reels / YouTube shorts.
//   node marketing/ads/generate_video_30.mjs
// Renders deterministic frames via Chromium, stitches to MP4 with ffmpeg.
// Output: marketing/ads/out/routeforge-ad-30.mp4
...
const FRAMES = join(__dirname, ".frames_30");
...
.eyebrow{font-size:34px;font-weight:700;color:#2ecf6e;letter-spacing:3px;text-transform:uppercase}
  ...
  <div id="sHook" class="layer center" style="padding:0 70px">
    <span id="hookEyebrow" class="eyebrow" style="margin-bottom:36px">Every race has a story</span>
  ...
  <div id="sSteps" class="layer center" style="padding:0 90px;align-items:flex-start;gap:60px">
    <h1 style="font-size:78px;text-align:left;align-self:flex-start">How it works</h1>
  ...
  <div id="sStat" class="layer center" style="gap:20px;padding:0 70px">
    ...
    <span style="font-size:40px;color:#9fb4a6;font-weight:600;margin-top:20px">RouteForge finds exactly where — and why.</span>
  </div>
...
const mp4 = join(OUT, "routeforge-ad-30.mp4");
execFileSync(FFMPEG, [
...
rmSync(FRAMES, { recursive: true, force: true });
console.log("✓ routeforge-ad-30.mp4 (1080×1920, " + DURATION + "s)");
